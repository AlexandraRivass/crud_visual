...
<head>
    ...
    <title>Propietarios — Fauna Granada</title>
...
<body>
                ...
                <div class="col">
                    <nav class="navbar navbar-expand-lg px-3">
                        <button class="navbar-toggler ms-auto" type="button" data-bs-toggle="collapse"
                                data-bs-target="#navPrincipal" aria-controls="navPrincipal" aria-expanded="false" aria-label="Abrir menú">
                        ...
                        <div class="collapse navbar-collapse" id="navPrincipal">
                            <ul class="navbar-nav gap-1 ms-auto align-items-center">
                                ...
                                <li class="nav-item dropdown">
                                    <a class="nav-link nav_pill nav_mascota dropdown-toggle" href="./tabla_mascotas.php" id="dropMascotas" role="button" data-bs-toggle="dropdown" aria-expanded="false">
    ...
    <div class="pagina_interior">
        <div class="cabecera_pagina mb-3">
            <h1 class="cabecera_titulo"><i class="fa-solid fa-user-tie me-2"></i>Propietarios</h1>
        ...
        <form method="GET" action="" class="barra_filtros">
            ...
            <input type="text" name="telef"    placeholder="Teléfono..." value="<?= htmlspecialchars($fil_telef) ?>">
            <button type="submit" class="btn_buscar"><i class="fa-solid fa-magnifying-glass me-1"></i>Buscar</button>
            <a href="./tabla_propietarios.php" class="btn_limpiar"><i class="fa-solid fa-xmark me-1"></i>Limpiar</a>
        </form>

        <div class="barra_acciones">
            <a href="./form_propietario.html" class="btn_agregar"><i class="fa-solid fa-plus"></i> Añadir propietario</a>
            <?php if ($hay_filtros): ?><span class="texto_resultados"><?= count($propietarios) ?> resultado(s)</span><?php endif; ?>
        </div>

        <?php if (isset($_GET['mensaje']) && $_GET['mensaje'] == 'eliminado'): ?>
            <div class="aviso_ok"><i class="fa-solid fa-check me-1"></i>Propietario eliminado correctamente.</div>
        <?php endif; ?>
        <?php if (isset($_GET['mensaje']) && $_GET['mensaje'] == 'actualizado'): ?>
            <div class="aviso_ok"><i class="fa-solid fa-check me-1"></i>Propietario actualizado correctamente.</div>
        <?php endif; ?>

        <table class="tabla_datos">
            <thead>
                <tr>
                    <th>ID</th><th>Nombre</th><th>Apellido</th><th>Teléfono</th><th>Email</th><th>DNI</th><th>Acciones</th>
                </tr>
            </thead>
            <tbody>
                <?php if (!empty($propietarios)): foreach ($propietarios as $fila): ?>
                <tr>
                    ...
                    <td style="white-space:nowrap;">
                        <a class="accion_modificar" href="../processes/validaciones/php/editar/editar_propietario.php?id=<?= $fila['id_propietario'] ?>"><i class="fa-solid fa-pen"></i> Modificar</a>
                        <a class="accion_eliminar ms-1" href="../processes/validaciones/php/eliminar/eliminar_propietario.php?id_propietario=<?= $fila['id_propietario'] ?>" onclick="return confirm('¿Seguro que quieres eliminar este propietario?')"><i class="fa-solid fa-trash"></i> Eliminar</a>
                    </td>
                </tr>
                <?php endforeach; else: ?>
                <tr><td colspan="7" style="text-align:center;padding:20px;color:#999;">No hay propietarios registrados.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
